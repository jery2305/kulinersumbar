                    ...
                    <div class="flex flex-col md:flex-row bg-white border-l-4 shadow-lg rounded-lg p-6 
                        ...
                        <div class="md:w-1/3 mb-4 md:mb-0 md:pr-6 border-r md:border-r-gray-200">
                            ...
                            @if(in_array($order->status, ['Diproses', 'Dikirim', 'Selesai']) && $order->resi)
                                <p class="text-xs mt-2 text-blue-600 bg-blue-50 px-2 py-1 rounded inline-block">
                                    Resi: {{ $order->resi }}
                                </p>
                            @endif
                        ...
                        <div class="md:w-2/3 md:pl-6">
                            ...
                            <!-- Upload Bukti (COD saat Dikirim) -->
                            @if(strtolower($order->pembayaran) === 'cod' && strtolower($order->status) === 'dikirim' && !$order->bukti_pembayaran)
                                <form action="{{ route('orders.uploadBukti', $order->id) }}" method="POST" enctype="multipart/form-data" class="mt-4">
                                    @csrf
                                    <p class="text-red-600 mb-1 font-semibold">⚠️ Silakan upload bukti pembayaran COD.</p>
                                    <label class="block text-sm mb-1 font-medium">Upload Bukti Pembayaran:</label>
                                    <input type="file" name="bukti" accept="image/*" required class="block w-full p-2 border rounded text-sm">
                                    <button type="submit" class="mt-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm rounded">
                                        Kirim Bukti
                                    </button>
                                </form>
                            @endif
                            ...
                            @if($order->status === 'Menunggu Pembayaran' && $order->pembayaran !== 'COD')
                                ...
                                @else
                                    <form action="{{ route('orders.uploadBukti', $order->id) }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <button type="submit" class="mt-2 px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white text-sm rounded">
                                            Kirim Bukti
                                        </button>
                                    </form>
                            ...
                            <div class="mt-4 space-y-3">
                                @if($order->status === 'Dikirim' && $order->bukti_pembayaran)
